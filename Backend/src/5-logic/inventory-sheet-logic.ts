...
async function addInventoryItemToSheet(sheetId: string, item: any): Promise<any> {
...
async function getInventorySheetSummary(id: string): Promise<any | null> {
    ...
    const inWarehouseItems = items.filter(item => item.status === "not_assigned").length;
...
async function getInventorySheetFull(
    ...
    const usersMap = new Map(
        users.map(user => [user.personalNumber, user])
    );
    ...
    return {
        ...
        counts: {
            ...
            notReportedCount: rows.filter(row => row.dailyReport.status === "not_reported").length,
            assignedCount: rows.filter(row => row.status === "assigned").length,
            inWarehouseCount: rows.filter(row => row.status === "not_assigned").length,
